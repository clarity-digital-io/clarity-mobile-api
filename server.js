...
const onAuthRealm = async (user) => {
	...
	try {
		...
		const config = { 	sync: { user: user, url: REALM_URL + '/~/userRealm', fullSynchronization: true, validate_ssl: false },  schema: [ResponseSchema, FormSchema, QuestionSchema, AnswerSchema] };
